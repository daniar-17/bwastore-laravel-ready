                        ...
                        <form action="{{ route('product.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="users_id" class="form-control">
                                            @foreach ($users as $user)
                                                <option value="{{ $user->id }}" {{ $user->id == $item->users_id ? 'selected' : '' }}>{{ $user->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="categories_id" class="form-control">
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}" {{ $category->id == $item->categories_id ? 'selected' : '' }}>{{ $category->nama }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col text-right">
                                    <a href="{{ route('product.index') }}" class="btn btn-secondary px-5 mr-2">
                                        Cancel
                                    </a>
